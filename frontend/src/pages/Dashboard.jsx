...
const StatCard = ({ label, value, sub, icon, color = 'accent', loading }) => {
  const colors = {
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {loading ? (
            ...
            <p className={`text-3xl font-black font-mono ${c.text} leading-none`}>{value ?? '—'}</p>
          )}
...
const Badge = ({ status }) => {
  const isActive = status?.toLowerCase() === 'active' || status?.toLowerCase() === 'ongoing';
  return (
...
const Dashboard = () => {
  ...
  useEffect(() => {
    getConflicts({ page: 1, limit: 6, sort: '-startYear' })
      .then((data) => {
        // handle both { data: [...] } and [...] shapes
        const list = Array.isArray(data) ? data : (data?.data ?? data?.conflicts ?? []);
        setConflicts(list);
      })
      .catch(() => setConflicts([]))
  ...
  const maxRegionCount = Math.max(...regions.map((r) => r.count ?? r.value ?? 1), 1);
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            value={stats?.totalConflicts ?? stats?.total}
            sub="In database"
            color="accent"
          ...
          <StatCard
            ...
            value={stats?.activeConflicts ?? stats?.active}
          ...
          <StatCard
            ...
            value={stats?.avgGDPImpact != null ? `${stats.avgGDPImpact.toFixed(1)}%` : stats?.averageGDPImpact != null ? `${Number(stats.averageGDPImpact).toFixed(1)}%` : null}
          ...
          <StatCard
            label="Avg War Cost"
            value={stats?.avgWarCost != null ? `$${Number(stats.avgWarCost).toLocaleString()}B` : null}
            sub="Billions USD"
            color="green"
            loading={loadingStats}
        ...
        <div className="lg:col-span-3 bg-card border border-border rounded overflow-hidden shadow-card">
          ...
          <div className="overflow-x-auto">
            {loadingConflicts ? (
              ...
              <div className="p-10 text-center text-muted text-sm">No conflict data available.</div>
            ) : (
                    ...
                    <tr key={c._id ?? i} className="border-b border-border/50 last:border-0 hover:bg-ink-950/40 transition-colors">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-ink-100 font-medium truncate max-w-[160px]">
                            {c.conflictName || c.name || '—'}
                          </span>
                        </div>
                      </td>
                      <td className="px-5 py-3 text-sm text-muted">{c.region || '—'}</td>
                      <td className="px-5 py-3"><Badge status={c.status} /></td>
                      <td className="px-5 py-3 text-right text-sm font-mono font-semibold text-accent-400">
                        {c.gdpImpact != null ? `${c.gdpImpact > 0 ? '+' : ''}${c.gdpImpact}%` : '—'}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded overflow-hidden shadow-card flex flex-col">
          ...
          <div className="p-5 flex-1 flex flex-col gap-4">
            {regions.length === 0 ? (
              ...
              regions.map((r, i) => {
                const label = r.region ?? r._id ?? r.name ?? `Region ${i + 1}`;
                const count = r.count ?? r.value ?? 0;
                return (
                  <div key={label}>
                    <div className="flex justify-between text-xs mb-1.5">
                      <span className="text-ink-300 truncate">{label}</span>
                      <span className="text-accent-400 font-mono font-semibold ml-2">{count}</span>
                    </div>
